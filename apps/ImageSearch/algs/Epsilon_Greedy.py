...
class MyAlg:
    ...
    def getQuery(self, butler, participant_uid, first_query_flag=False):
        if first_query_flag:
            ...
            utils.debug_print('Running Epsilon Greedy')
            epsilon = butler.participants.get(uid=participant_uid, key='epsilon')
            if np.random.rand() >= epsilon:
                ...
                do_not_ask = butler.participants.get(uid=participant_uid, key='_bo_do_not_ask')
                n = butler.participants.get(uid=participant_uid, key = 'n')
                i_x = np.random.choice(np.setdiff1d(range(n), do_not_ask))
                butler.participants.append(uid=participant_uid,
                                       key='_bo_do_not_ask', value=i_x)
                return i_x
